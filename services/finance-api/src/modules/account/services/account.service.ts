...
export class AccountService {
  async createAccount(input: ICreateAccountInput): Promise<IAccountInfo> {
    try {
      const { biodataId, bankId, accountNumber, bvn, accountName } = input;
      
      const biodataExists = await prisma.biodata.findUnique({ 
        where: { id: biodataId } 
      });
      ...
      if (!biodataExists.isVerified) {
        throw new ApiError('The biodata entry is not verified. AccountInfo cannot be created', 400);
      }
      ...
      if (!bankExists) {
        throw new ApiError('The requested bank entry does not exist.', 404);
      }
      
      // Get bank code from bank record
      const bankCode = bankExists.code;
      
      // Validate account number format
      if (!bankAccountVerification.validateAccountNumber(accountNumber)) {
        throw new ApiError('Invalid account number format. Must be 10 digits.', 400);
      }
      
      // try {
      const verificationResult = await bankAccountVerification.verifyAccount({
      ...
      if (verificationResult.status === 'error') {
        throw new ApiError(
          'Account information not available. Please try again or verify the submitted details.', 
          500
        )
        // throw { 
      ...
      const duplicateBankAccountNumber = await prisma.accountInfo.findFirst({ 
        where: { accountNumber } 
      });
      
      if (duplicateBankAccountNumber) {
        throw new ApiError('A record with this account number already exists.', 409)
      }
      
      const duplicateBvn = await prisma.accountInfo.findFirst({ 
        where: { bvn } 
      });
      
      if (duplicateBvn) {
        throw new ApiError('BVN already exists: A record with this BVN already exists.', 409)
      }
      
      return await prisma.$transaction(async (tx) => {
        // Create account info with verified details
        const accountInfo = tx.accountInfo.create({
          data: { 
            biodataId, 
            bankId, 
            accountNumber, 
        ...
        return mapToAccountInfo(accountInfo);
      }); 
    } catch (error: any) {
      ...
      // If it's a bank verification error with status and message
