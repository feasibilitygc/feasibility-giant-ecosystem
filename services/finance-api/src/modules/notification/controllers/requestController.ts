...
import { RequestError, RequestErrorCodes } from '../../request/errors/request.error';
...
export class RequestController {
  ...
  async updateRequest(req: AuthenticatedRequest, res: Response) {
    try {
      ...
      const request = await requestService.updateRequest(id as string, updateData, adminId);
      res.json(request);
    } catch (error) {
      if (error instanceof RequestError) {
        res.status(error.status).json({ error: { code: error.code, message: error.message } });
