import { prisma, Prisma, PersonalSavingsStatus, TransactionType, RequestType, RequestStatus, TransactionModule, TransactionStatus, RequestModule, MembershipStatus } from '@/prisma';
...
export class PersonalSavingsService {
    ...
    async requestPersonalSavingsCreation(data: {
        ...
        try {
            ...
            if (member.membershipStatus !== MembershipStatus.ACTIVE) {
                throw new ApiError('Only members with ACTIVE status can request for personal savings plans', 403);
            }
            ...
            if (!planType.isActive) {
                throw new ApiError('This savings plan type is no longer available', 400);
            }
            ...
            logger.error(`Error requesting personal savings plan creation: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
            throw new ApiError('Failed to request personal savings plan creation', 500);
        }
    }
     ...
     */
    async getAllPersonalSavings(queryParams: IPersonalSavingsQueryParams, userId: string, isAdmin: boolean) {
        try {
            const { page = 1, limit = 12, erpId, status, sort = 'desc', includePending = true } = queryParams;
                    ...
                    include: {
                        ...
                        requests: {
                            where: {
                                ...
                                status: {
                                    in: [RequestStatus.PENDING, RequestStatus.IN_REVIEW, RequestStatus.REVIEWED, RequestStatus.APPROVED]
                            }
                        },
                        orderBy: {
                            createdAt: 'desc'
        ...
        if (includePending) {
            ...
            const pendingWhere: any = {
                type: RequestType.PERSONAL_SAVINGS_CREATION,
                status: {
                    in: [RequestStatus.PENDING, RequestStatus.IN_REVIEW, RequestStatus.REVIEWED]
                }
            };
        ...
        const formattedPlans = plans.map((plan: any) => ({
            ...
            transactions: plan.transactions.map((transaction: any) => ({
                id: transaction.id,
            ...
            pendingWithdrawal: plan.requests.length > 0 ? {
                requestId: plan.requests[0].id,
                status: plan.requests[0].status,
                requestedAt: plan.requests[0].createdAt,
                amount: typeof plan.requests[0].content === 'object' && plan.requests[0].content !== null ? (plan.requests[0].content as any).amount || null : null,
                currentApprovalLevel: plan.requests[0].nextApprovalLevel,
                approvalSteps: plan.requests[0].approvalSteps.map((step: any) => ({
                    level: step.level,
                    status: step.status,
        ...
        const formattedPendingRequests = pendingCreationRequests.map((request: any) => {
            ...
            return {
                ...
                approvalSteps: request.approvalSteps.map((step: { level: any; status: any; approverRole: any; approvedAt: any; }) => ({
                    level: step.level,
                    status: step.status,
                    approverRole: step.approverRole,
                    approvedAt: step.approvedAt
        ...
        const totalCombined = total + (includePending ? pendingCreationRequests.length : 0);
        ...
        return {
            ...
            meta: {
                ...
                activeCount: plans.filter((p: any) => p.status === PersonalSavingsStatus.ACTIVE).length,
                closedCount: plans.filter((p: any) => p.status === PersonalSavingsStatus.CLOSED).length,
            }
        };        
    } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
        logger.error(`Error fetching personal savings plans: ${errorMessage}`, { error });
    ...
    async getPersonalSavingsById(id: string, userId: string, isAdmin: boolean): Promise<IPersonalSavingsResponse> {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error fetching personal savings plan: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
    ...
    async processDeposit(id: string, amount: number, userId: string, description?: string): Promise<IPersonalSavingsResponse> {
        try {
            ...
            if (plan.status !== PersonalSavingsStatus.ACTIVE) {
                throw new ApiError('Cannot deposit to a closed or suspended savings plan', 400);
            }
            ...
            return await this.prisma.$transaction(async (tx) => {
                ...
                const transaction = await tx.transaction.create({
                    data: {
                        ...
                        description: description || `Deposit to personal savings plan: ${plan.planName || 'Unnamed plan'}`,
                        initiatedBy: userId,
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error processing personal savings deposit: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
    ...
    async requestWithdrawal(id: string, amount: number, userId: string, reason?: string): Promise<any> {
        try {
            ...
            if (plan.status !== PersonalSavingsStatus.ACTIVE) {
                throw new ApiError('Cannot withdraw from a closed or suspended savings plan', 400);
            }
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error requesting personal savings withdrawal: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
     ...
     */
    async getTransactionHistory(planId: string, queryParams: IPersonalSavingsTransactionQueryParams, userId: string, isAdmin: boolean) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error fetching transaction history: ${errorMessage}`, { error });
    ...
    async getBalanceHistory(planId: string, startDate?: string, endDate?: string, userId?: string, isAdmin?: boolean): Promise<IPersonalSavingsBalanceHistory> {
        try {
            ...
            if (transactions.length > 0) {
                ...
                const isDebit = transactions[0].transactionType === TransactionType.PERSONAL_SAVINGS_WITHDRAWAL;
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error fetching balance history: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
    ...
    async closePlan(id: string, userId: string): Promise<IPersonalSavingsResponse> {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error closing personal savings plan: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
     ...
     */
    async getMemberSummary(erpId: string, userId?: string, isAdmin?: boolean): Promise<{
        totalSaved: number;
        ...
        try {
            ...
            if (userId && isAdmin !== undefined && !isAdmin) {
                ...
                // If the member isn't the current user, deny access
                if (erpId !== user.biodata.erpId) {
                    throw new ApiError('You do not have permission to access this member\'s summary', 403);
                }
            }
            ...
            return {
                totalSaved: Number(totalSaved._sum.amount || 0), // Sum of all deposit transactions
                totalWithdrawals: Number(totalWithdrawals._sum.amount || 0), // Sum of all withdrawal transactions
                currentBalance: Number(currentBalance._sum.currentBalance || 0), // Current balance from personalSavings table
                plansCount,
                activePlans: formattedPlans,
            };        
        } catch (error: unknown) {
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error getting member savings summary: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
    ...
    async getAdminDashboardData(userId: string) {
        try {
            ...
            const [
                ...
                this.prisma.transaction.findMany({
                    where: {
                        module: TransactionModule.SAVINGS,
                        transactionType: {
                            in: [TransactionType.PERSONAL_SAVINGS_DEPOSIT, TransactionType.PERSONAL_SAVINGS_WITHDRAWAL]
                        }
                    },
                    include: {
                        personalSavings: {
            ...
            const formattedTransactions = recentTransactions.map((tx: any) => ({
                ...
                planDescription: tx.personalSavings?.planType?.description || 'No description',
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            logger.error(`Error getting admin dashboard data: ${errorMessage}`, { error });
            if (error instanceof ApiError) {
                throw error;
            }
