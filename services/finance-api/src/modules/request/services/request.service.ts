                    ...
                    if (!existingPlan) {
                        const newPlan = await tx.personalSavings.create({
                            data: {
                                ...
                                targetAmount: request.content.targetAmount ? new (await import('@prisma/client/runtime/client')).Decimal(request.content.targetAmount) : null,
                                currentBalance: new (await import('@prisma/client/runtime/client')).Decimal(0),
                                status: 'ACTIVE',
                ...
                if (request.content && request.personalSavingsId) {
                    ...
                    if (plan) {
                        ...
                        if (!plan.currentBalance.lessThan(amount)) {
                            ...
                            const newBalance = plan.currentBalance.sub(new (await import('@prisma/client/runtime/client')).Decimal(amount));
                            ...
                            await tx.transaction.create({
                                data: {
                                    ...
                                    baseType: 'DEBIT',
                                    module: 'SAVINGS',
                                    amount: new (await import('@prisma/client/runtime/client')).Decimal(amount),
