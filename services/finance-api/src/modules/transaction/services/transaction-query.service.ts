import { Prisma, prisma, TransactionStatus, TransactionModule } from '@/prisma';
...
export class TransactionQueryService {
    ...
    try {
      ...
      const whereClause = {
        OR: [
          ...
          ...(biodata.savings.map((savings: { id: any; }) => ({
            OR: [
          ...
          ...(biodata.shares.map((shares: { id: any; }) => ({
            OR: [
              { sharesId: shares.id },
  ...
  async userHasAccessToEntity(userId: string, entityType: string, entityId: string): Promise<boolean> {
    try {
      ...
      switch (entityType) {
        case 'LOAN':
          return user.biodata.loans.some((loan: { id: string; }) => loan.id === entityId);
        case 'SAVINGS':
          return user.biodata.savings.some((savings: { id: string; }) => savings.id === entityId);
        case 'SHARES':
          return user.biodata.shares.some((shares: { id: string; }) => shares.id === entityId);
        case 'BIODATA':
  ...
  private async getLoanIdsForUser(userId: string): Promise<string[]> {
    ...
    return loans.map((loan: { id: any; }) => loan.id);
  }
  ...
  private async getSavingsIdsForUser(userId: string): Promise<string[]> {
    ...
    return savings.map((savings: { id: any; }) => savings.id);
  }
  ...
  private async getSharesIdsForUser(userId: string): Promise<string[]> {
    ...
    return shares.map((shares: { id: any; }) => shares.id);
  }
}