import { prisma, Prisma, TransactionStatus, TransactionType, TransactionModule } from '@/prisma';
import { 
  ...
  async generateTransactionReport(filters: TransactionReportFilters): Promise<any> {
    try {
      ...
      return {
        ...
        summary: {
          totalTransactions: transactions.length,
          totalAmount: transactions.reduce((sum: any, txn: { amount: { toNumber: () => any; }; }) => sum + txn.amount.toNumber(), 0),
          dateRange: {
