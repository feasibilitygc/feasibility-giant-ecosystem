...
import { RequestStatus, RequestType } from '@/prisma';
...
export class UserController {
    ...
    try {
      ...
      if (!biodataId) {
        return res.status(400).json({
          status: 'error',
          message: 'biodataId cookie is missing. Please verify your biodata first.'
        });
      }
  ...
  async updateUsername(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await this.userService.updateUsername(userId, validatedData);
  ...
  async approveUsernameUpdate(req: AuthRequest, res: Response, next: NextFunction) {
    try {
  ...
  async getUserModuleAccess(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.params.id as string;
  ...
  async updateUserRole(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const { userRoleId } = req.params as { userRoleId: string };
      ...
      const userRole = await this.userService.updateUserRole(userRoleId, updates);
      res.json({
        status: 'success',
        data: userRole
      });
  ...
  async invalidateAllSessions(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user.id;
  ...
  async getAssignedRequests(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await this.userService.getAssignedRequests(userId, queryParams);
  ...
  async getInitiatedRequests(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user.id;
  ...
  async getApprovedRequests(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const userId = req.user.id;
